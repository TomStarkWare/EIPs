...
> :information\_source: [**ERC1820**](https://eips.ethereum.org/EIPS/eip-1820) **has superseded** [**ERC820**](https://eips.ethereum.org/EIPS/eip-820)**.** :information\_source:  
> [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) fixes the incompatibility in the [ERC165](https://eips.ethereum.org/EIPS/eip-165) logic which was introduced by the Solidty 0.5 update.  
> Have a look at the [official announcement](https://github.com/ethereum/EIPs/issues/820#issuecomment-464109166), and the comments about the [bug](https://github.com/ethereum/EIPs/issues/820#issuecomment-452465748) and the [fix](https://github.com/ethereum/EIPs/issues/820#issuecomment-454021564).  
> Apart from this fix, [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) is functionally equivalent to [ERC820](https://eips.ethereum.org/EIPS/eip-820).
>
> :warning: [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) MUST be used in lieu of [ERC820](https://eips.ethereum.org/EIPS/eip-820). :warning:
...
This standard keeps backward compatibility with [ERC165](https://eips.ethereum.org/EIPS/eip-165).
...
Interfaces with zeroes \(`0`\) as the last 28 bytes are considered [ERC165](https://eips.ethereum.org/EIPS/eip-165) interfaces, and this registry SHALL forward the call to the contract to see if it implements the interface.

This contract also acts as an [ERC165](https://eips.ethereum.org/EIPS/eip-165) cache to reduce gas consumption.

## Motivation

There have been different approaches to define pseudo-introspection in Ethereum. The first is [ERC165](https://eips.ethereum.org/EIPS/eip-165) which has the limitation that it cannot be used by regular accounts. The second attempt is [ERC672](https://github.com/ethereum/EIPs/issues/672) which uses reverse [ENS](https://ens.domains/). Using reverse [ENS](https://ens.domains/) has two issues. First, it is unnecessarily complicated, and second, [ENS](https://ens.domains/) is still a centralized contract controlled by a multisig. This multisig theoretically would be able to modify the system.

This standard is much simpler than [ERC672](https://github.com/ethereum/EIPs/issues/672), and it is _fully_ decentralized.
...
### [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) Registry Smart Contract

> This is an exact copy of the code of the [ERC1820 registry smart contract](https://github.com/0xjac/ERC1820/blob/master/contracts/ERC1820Registry.sol).
...
This contract is going to be deployed using the keyless deployment method---also known as [Nick](https://github.com/Arachnid/)'s method---which relies on a single-use address. \(See [Nick's article](https://medium.com/@weka/how-to-send-ether-to-11-440-people-187e332566b7) for more details\). This method works as follows:

1. Generate a transaction which deploys the contract from a new random account.
   * This transaction MUST NOT use [EIP155](https://eips.ethereum.org/EIPS/eip-155) in order to work on any chain.
...
If the interface is part of a standard, it is best practice to explicitly state the interface name and link to this published [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) such that other people don't have to come here to look up these rules.
...
#### [**ERC165**](https://eips.ethereum.org/EIPS/eip-165) **Compatible Interfaces**

> The compatibility with [ERC165](https://eips.ethereum.org/EIPS/eip-165), including the [ERC165 Cache](eip-1820.md#erc165-cache), has been designed and developed with [William Entriken](https://github.com/fulldecent).

Any interface where the last 28 bytes are zeroes \(`0`\) SHALL be considered an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface.

[**ERC165**](https://eips.ethereum.org/EIPS/eip-165) **Lookup**

Anyone can explicitly check if a contract implements an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface using the registry by calling one of the two functions below:

```text
function implementsERC165Interface(address _contract, bytes4 _interfaceId) public view returns (bool)
```

Checks whether a contract implements an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface or not.

If the result is not cached a direct lookup on the contract address is performed.

_NOTE_: If the result is not cached or the cached value is out-of-date, the cache MUST be updated manually by calling `updateERC165Cache` with the contract address. \(See [ERC165 Cache](eip-1820.md#erc165-cache) for more details.\)

> **identifier:** `f712f3e8`  
> **parameters**  
> `_contract`: Address of the contract to check.  
> `_interfaceId`: [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface to check.  
...
Checks whether a contract implements an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface or not without using nor updating the cache.
...
[**ERC165**](https://eips.ethereum.org/EIPS/eip-165) **Cache** 

Whether a contract implements an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface or not can be cached manually to save gas.

If a contract dynamically changes its interface and relies on the [ERC165](https://eips.ethereum.org/EIPS/eip-165) cache of the [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) registry, the cache MUST be updated manually---there is no automatic cache invalidation or cache update. Ideally the contract SHOULD automatically update the cache when changing its interface. However anyone MAY update the cache on the contract's behalf.
...
> **parameters**  
> `_contract`: Address of the contract for which to update the cache.  
> `_interfaceId`: [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface for which to update the cache.

#### **Private User-defined Interfaces** <a id="erc165-cache"></a>
...
For any address to set a contract as the interface implementation, it must call the following function of the [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) registry:
...
Only the `manager` defined for that address can set it. \(Each address is the manager for itself, see the [manager](eip-1820.md#manager) section for more details.\)
...
_NOTE_: The `_interfaceHash` MUST NOT be an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface---it MUST NOT end with 28 zeroes \(`0`\).
...
Anyone MAY query the [ERC1820](https://eips.ethereum.org/EIPS/eip-1820) Registry to obtain the address of a contract implementing an interface on behalf of some address using the `getInterfaceImplementer` function.
...
_NOTE_: If the last 28 bytes of the `_interfaceHash` are zeroes \(`0`\), then the first 4 bytes are considered an [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface and the registry SHALL forward the call to the contract at `_addr` to see if it implements the [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface \(the first 4 bytes of `_interfaceHash`\). The registry SHALL also cache [ERC165](https://eips.ethereum.org/EIPS/eip-165) queries to reduce gas consumption. Anyone MAY call the `erc165UpdateCache` function to update whether a contract implements an interface or not.
...
The registry can also act as a [ERC165](https://eips.ethereum.org/EIPS/eip-165) cache in order to save gas when looking up if a contract implements a specific [ERC165](https://eips.ethereum.org/EIPS/eip-165) interface. This cache is intentionally kept simple, without automatic cache update or invalidation. Anyone can easily and safely update the cache for any interface and any contract by calling the `updateERC165Cache` function.
...
This standard is backward compatible with [ERC165](https://eips.ethereum.org/EIPS/eip-165), as both methods MAY be implemented without conflicting with each other.

## Test Cases

Please check the [0xjac/ERC1820](https://github.com/0xjac/ERC1820) repository for the full test suite.

## Implementation

The implementation is available in the repo: [0xjac/ERC1820](https://github.com/0xjac/ERC1820).
