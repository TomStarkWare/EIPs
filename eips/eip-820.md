...
### [ERC820](https://eips.ethereum.org/EIPS/eip-820) Registry Smart Contract

> This is an exact copy of the code of the [ERC820 registry smart contract](https://github.com/jbaylina/ERC820/blob/master/contracts/ERC820Registry.sol).
...
If the interface is part of a standard, it is best practice to explicitly state the interface name and link to this published [ERC820](https://eips.ethereum.org/EIPS/eip-820) such that other people don't have to come here to look up these rules.
...
> The compatibility with [ERC165](https://eips.ethereum.org/EIPS/eip-165), including the [ERC165 Cache](eip-820.md#erc165-cache), has been designed and developed with [William Entriken](https://github.com/fulldecent).
...
_NOTE_: The result is cached. If the cache is out of date, it MUST be updated by calling `updateERC165Cache`. \(See [ERC165 Cache](eip-820.md#erc165-cache) for more details.\)
...
If a contract dynamically changes its interface and relies on the [ERC165](https://eips.ethereum.org/EIPS/eip-165) cache of the [ERC820](https://eips.ethereum.org/EIPS/eip-820) registry, the cache MUST be updated manually---there is no automatic cache invalidation or cache update. Ideally the contract SHOULD automatically update the cache when changing its interface. However anyone MAY update the cache on the contract's behalf.
...
For any address to set a contract as the interface implementation, it must call the following function of the [ERC820](https://eips.ethereum.org/EIPS/eip-820) registry:
...
Only the `manager` defined for that address can set it. \(Each address is the manager for itself, see the [manager](eip-820.md#manager) section for more details.\)
...
Anyone MAY query the [ERC820](https://eips.ethereum.org/EIPS/eip-820) Registry to obtain the address of a contract implementing an interface on behalf of some address using the `getInterfaceImplementer` function.
...
Please check the [jbaylina/ERC820](https://github.com/jbaylina/ERC820) repository for the full test suite.

## Implementation

The implementation is available in the repo: [jbaylina/ERC820](https://github.com/jbaylina/ERC820).
